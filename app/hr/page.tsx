...
export default async function HRPage() {
  ...
  // 팀별 그룹핑
  const teams = Array.from(new Set((interns || []).map((i: any) => i.department || '기타'))).sort()
      ...
      <div style={{ maxWidth: 960, margin: '28px auto', padding: '0 20px 80px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: '#999', marginTop: 2 }}>전사 인턴 레포트 및 면담 보고서 열람</div>
          </div>
          {profile.is_admin && (
        ...
        {!interns?.length ? (
          ...
          <>
            {teams.map(team => {
              const teamInterns = (interns as Profile[]).filter(i => (i.department || '기타') === team)
              return (
                <div key={team}>
                  {/* 팀 구분 헤더 */}
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10, marginTop: 8, paddingLeft: 4 }}>
                    {team} 팀
                  </div>
                  ...
                  {teamInterns.map(intern => {
                    ...
                    return (
                      <div key={intern.id} className="card" style={{ marginBottom: 16 }}>
