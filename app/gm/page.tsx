...
export default async function GMPage() {
  const { profile } = await requireProfile('gm')
    ...
    <>
      ...
      <div style={{ maxWidth: 1080, margin: '28px auto', padding: '0 24px 80px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: '#1F4E79' }}>All Reports — GM View</div>
            <div style={{ fontSize: 13, color: '#999', marginTop: 2 }}>
              All intern reports & interview records · {internsData.length} {internsData.length === 1 ? 'intern' : 'interns'}
            </div>
          </div>
          {profile.is_admin && (
