                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: '#777', marginTop: 2 }}>
                      {intern.department}{mentor ? ` · Mentor: ${mentor}` : ''}
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {internReports.map((r: Report) => (
                      <Link key={r.id} href={r.status === 'completed' ? `/report/${r.id}/print` : '#'} target={r.status === 'completed' ? '_blank' : undefined} style={{ textDecoration: 'none' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#F9FAFB', border: '1px solid #E8EDF3', borderRadius: 8, padding: '12px 16px', cursor: r.status === 'completed' ? 'pointer' : 'default', opacity: r.status === 'draft' ? 0.6 : 1 }}>
                          <div>
