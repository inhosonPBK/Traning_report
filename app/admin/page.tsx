...
export default async function AdminPage() {
  ...
  // Approved users (for display + mentor list)
    ...
    <>
      ...
      <div style={{ maxWidth: 860, margin: '28px auto', padding: '0 20px 80px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          <Link href="/manager" style={{ fontSize: 13, color: '#1F4E79', textDecoration: 'none', fontWeight: 600 }}>
            View All Reports →
          </Link>
        </div>
                  ...
                  <tr key={u.id} style={{ borderBottom: '1px solid #E8EDF3' }}>
                    ...
                    <td style={{ padding: '10px 10px' }}>
                      <span style={{
                        ...
                        background: u.role === 'intern' ? '#EEF2FF' : u.role === 'mentor' ? '#E2EFDA' : '#FCE4D6',
                        color: u.role === 'intern' ? '#3730A3' : u.role === 'mentor' ? '#375623' : '#C55A11',
                      }}>{u.role}</span>
                    </td>
                    <td style={{ padding: '10px 10px', color: '#666' }}>{u.department || '—'}</td>
